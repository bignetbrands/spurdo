...
/**
 * Look up another user's numeric ID by their username (without @).
 * Cached in-process indefinitely. Family accounts rarely change handles
 * so caching aggressively is fine.
 */
const _userIdByName = new Map<string, string>();
export async function lookupUserIdByUsername(username: string): Promise<string | null> {
  const norm = username.replace(/^@/, "").toLowerCase();
  const cached = _userIdByName.get(norm);
  if (cached) return cached;
  if (isDryRun()) return `dryrun_${norm}`;
  const client = getClient();
  try {
    const r = await client.v2.userByUsername(norm);
    if (r.data?.id) {
      _userIdByName.set(norm, r.data.id);
      return r.data.id;
    }
  } catch {
    // protected accounts / suspended / non-existent — return null
  }
  return null;
}
...
/**
 * Fetch a user's recent original tweets. Filters out retweets, replies,
 * and quote tweets by default — we want originals to engage with.
 *
 * Used by the family-engage cron: picks a family account, fetches their
 * recent originals, replies to the freshest one we haven't engaged with.
 */
export async function fetchUserRecentTweets(opts: {
  userId: string;
  maxResults?: number;
  /** When true, include retweets/replies/quotes. Default: false (originals only). */
  includeNonOriginal?: boolean;
}): Promise<UserTweet[]> {
  if (isDryRun()) return [];

  const client = getClient();
  const max = Math.min(Math.max(opts.maxResults ?? 10, 5), 100);

  const exclude: ("replies" | "retweets")[] = opts.includeNonOriginal ? [] : ["replies", "retweets"];

  const tl = await client.v2.userTimeline(opts.userId, {
    max_results: max,
    exclude,
  ...
  for (const t of tl.data?.data ?? []) {
    ...
    if (!opts.includeNonOriginal && (isRetweet || isReply)) continue;
